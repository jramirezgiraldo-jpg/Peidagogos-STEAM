...
const app = {
    ...
    renderOvaContent() {
        ...
        let html = `
            ...
            <section class="ova-section">
                ...
                <button class="btn-mision-check" onclick="app.updateProgress(25); alert('✅ Misión en Cuaderno Completada!');">✅ Misión en Cuaderno Completada</button>
            </section>
            
            <section class="ova-section">
                <h3>🧩 Ordena los Conceptos (Drag & Drop)</h3>
                <div id="dnd-area" style="display: flex; justify-content: space-between; gap: 2rem;">
                    <!-- Columna Izquierda: Desordenadas -->
                    <div id="dnd-source" style="flex: 1; display: flex; flex-direction: column; gap: 1rem;"></div>
                    <!-- Columna Derecha: Zonas de Destino -->
                    <div id="dnd-target" style="flex: 1; display: flex; flex-direction: column; gap: 1rem;"></div>
                </div>
            </section>

            <section class="ova-section">
                <h3>📚 Profundización</h3>
                <p class="ova-text">${data.texto_deductivo}</p>
            </section>

            <section class="ova-section">
                <h3>📓 Cuaderno de Misión 2</h3>
                <ul class="ova-text">
                    ${data.actividades_cuaderno_2.map(a => `<li>${a}</li>`).join('')}
                </ul>
                <button class="btn-mision-check" onclick="app.updateProgress(25); alert('✅ Misión en Cuaderno 2 Completada!');">✅ Misión en Cuaderno Completada</button>
            </section>

            <section class="ova-section">
                <h3>🎯 Evaluación de Misión (ICFES)</h3>
                ${data.preguntas_icfes.map((q, i) => `
                    <div style="margin-bottom: 2rem; padding: 1.5rem; background: #f8fafc; border-radius: 12px;">
                        <p class="ova-text" style="font-style: italic; color: #475569;">${q.contexto}</p>
                        <p class="ova-text"><strong>${i+1}. ${q.enunciado}</strong></p>
                        ${Object.keys(q.opciones).map(k => `
                            <label class="icfes-option">
            ...
            <section class="ova-section">
                <h3>📝 Cierre de Selección Múltiple</h3>
            ...
            <section class="ova-section">
                <h3>✍️ Preguntas Abiertas de Síntesis</h3>
    ...
    initDragAndDrop(dndData) {
        const sourceContainer = document.getElementById('dnd-source');
        const targetContainer = document.getElementById('dnd-target');
        let sourceHtml = '';
        let targetHtml = '';
        dndData.forEach((item, idx) => {
            sourceHtml += `<div class="dnd-word" draggable="true" id="drag_${idx}" ondragstart="app.iniciarArrastre(event)">${item.palabra_desordenada.toUpperCase()}</div>`;
            targetHtml += `<div class="dnd-dropzone" id="drop_${idx}" data-correct="${item.palabra_correcta.toUpperCase()}" ondragover="event.preventDefault()" ondrop="app.soltarElemento(event)">Soltar aquí</div>`;
        });
