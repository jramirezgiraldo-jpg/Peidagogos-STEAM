...
const app = {
    ...
    initDragAndDrop(letrasData, frasesData) {
        ...
        letrasData.forEach((item, idx) => {
            ...
            htmlLetras += `<div>${item.letras.map((l, i) => `<span class="letra-tile" draggable="true" id="l_${idx}_${i}" ondragstart="app.dragStart(event)">${l}</span>`).join('')}</div>`;
        ...
        frasesData.forEach((item, idx) => {
            ...
            htmlFrases += `<div>${item.palabras.map((p, i) => `<span class="palabra-tile" draggable="true" id="p_${idx}_${i}" ondragstart="app.dragStart(event)">${p}</span>`).join('')}</div>`;
